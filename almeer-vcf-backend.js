 ...
 * ENV VARS:
 *   PORT=3788
 *   PAIRING_NUMBER=254712345678   ← your number, NO + sign
...
const PAIRING_NUMBER   = process.env.PAIRING_NUMBER   || '';  // e.g. 254712345678
...
async function startSock() {
  ...
  sock = makeWASocket({
    ...
    printQRInTerminal: !PAIRING_NUMBER, // show QR only when no pairing number
  ...
  sock.ev.on('connection.update', async ({ connection, lastDisconnect, qr }) => {
    ...
    /* ── PAIRING CODE ── 
       Runs once when socket first opens and number is not yet registered.
       We wait 3 seconds after 'open' so the socket is fully ready. */
    if (connection === 'open' && PAIRING_NUMBER && !sock.authState.creds.registered) {
      await delay(3000);
      try {
    ...
    if (connection === 'open') {
      ...
      console.log(`[ALMEER VCF] ► API: http://localhost:${PORT}`);
  ...
  /* 4️⃣ Collect incoming messages for bot-ping detection */
  sock.ev.on('messages.upsert', ({ messages }) => {
    for (const msg of messages) {
      if (!msg.message || msg.key.fromMe) continue;
      const jid = msg.key.remoteJid;
      if (pendingPings.has(jid)) {
        pendingPings.get(jid)(msg); // resolve the pending ping promise
      }
    }
  });
}
...
async function pingForBot(jid) {
  return new Promise(async (resolve) => {
    const timer = setTimeout(() => {
      ...
      resolve(false); // no reply within timeout = not a bot
    }, BOT_PING_TIMEOUT);

    pendingPings.set(jid, () => {
      clearTimeout(timer);
      pendingPings.delete(jid);
      resolve(true); // got a reply = bot detected
    });
...
app.post('/verify', async (req, res) => {
  ...
  try {
    ...
    /* Decision:
       ping ON  → must be on WA + have bot
       ping OFF → just needs to be on WA       */
